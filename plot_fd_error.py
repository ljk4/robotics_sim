# -*- coding: utf-8 -*-
"""FD acceleration error vs time (ID -> FD consistency check)."""
...
for i in range(n):
    ...
    vals = data[:, col[f"qdd_err_{upper_joints[i]}"]]
    ax.plot(t, vals, color=colors[i], linewidth=0.6)
    ax.axhline(y=0, color="gray", linewidth=0.5, linestyle="--")
    ax.set_title(labels[i], fontsize=8)
    if i == 0:
        ax.set_ylabel("qdd error (rad/s^2)", fontsize=9)
    ax.tick_params(labelsize=6)
    ax.grid(True, alpha=0.3)

out = os.path.join(os.path.dirname(__file__), "fd_error.png")
